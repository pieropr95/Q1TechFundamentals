...
def centroid_assignation(points, k, lock, max_iter=20): 
    global l_wcss
    #Initialization 
    ...
    lock.acquire()
    l_wcss.append((k,loss))
    lock.release()

def updatecentroids (points, centroid_idx, i, lock):
    global centroids
    lock.acquire()
    centroids[i] = np.mean(points[centroid_idx == i], axis = 0)
    lock.release()

def read(skipping, chunksize, lock):
    global dataset
...
if __name__ == '__main__':
    ...
    nchunks = 3 # CHANGEABLE PARAMETER
    ...
    # OPTION 2 READING: With threads:
    global dataset
    dataset = []
    start0 = time.time()
    th = []
    lock = threading.Lock()
    for i, skipping in enumerate(l_skipping):
        th.append(threading.Thread(name='th%s' %i, target= read, args=(skipping, chunksize, lock)))
        th[i].start()
    for i, skipping in enumerate(l_skipping):
        th[i].join()
    dataset = pd.concat(dataset)
    maxprice = max(dataset['price'])
    dataset = dataset.div(dataset.max(axis=0), axis=1)
    ...
    for i in range(len(possible_k)):
        th.append(threading.Thread(target= centroid_assignation, args= (points, possible_k[i], lock)))
        th[i].start()
    for i in range(len(possible_k)):
        th[i].join()
    ...
    max_iter = 20 # CHANGEABLE PARAMETER
    start = time.time()
    global centroids
    centroids = initialization(points, k)
    ...
    while np.not_equal(centroids, prev_centroids).any() and iteration < max_iter: #All centroids need to be equal to stop
        ...
        lock = threading.Lock()
        for i in range(k):
            th.append(threading.Thread(target= updatecentroids, args= (points, centroid_idx, i, lock)))
            th[i].start()
        for i in range(k):
            th[i].join()
        for i, centroid in enumerate(centroids):
